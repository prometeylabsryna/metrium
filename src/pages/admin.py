    ...
    def content_edit_hint(self, obj: StaticPage) -> str:
        ...
        parts: list[str] = [
            format_html(
                ...
                "<li>Нижче блоки <strong>у тому ж порядку, що на сайті</strong> "
                "(Hero → опис → вартість → картки → FAQ…).</li>"
                "<li>Відкрийте блок (напр. «2. Основний опис») і змініть текст у великому полі.</li>"
                "<li>Підпис над полем показує, що саме це: заголовок, абзац чи FAQ.</li>"
                "<li>Мова картки: <strong>{}</strong>. Іншу мову — кнопкою вище.</li>"
                "</ol>",
                lang_label,
            )
        ]
